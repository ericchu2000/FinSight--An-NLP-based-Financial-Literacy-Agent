...
from urllib.parse import quote_plus  # ✅ 用来构造带 num= 的搜索 URL
...
async def search(
    ...
    logger.info(
        f"[web_search] query={query!r}, options.limit={options.limit}, effective limit={limit}"
    )
        ...
        if not browser_was_provided:
            ...
            async with async_playwright() as p:
                ...
                return await _perform_search_with_browser(
                    browser, browser_was_provided, headless
                )
        else:
            return await _perform_search_with_browser(
                browser, browser_was_provided, headless
    ...
    async def _perform_search_with_browser(
        browser: Browser, browser_was_provided: bool, headless: bool = True
    ):
        """使用给定的浏览器搜索"""
        try:
            # 获取设备配置
            ...
            # ✅ 直接构造带 num=limit 的搜索 URL，而不是先打开首页再输入
            encoded_query = quote_plus(query)
            search_url = f"{selected_domain}/search?q={encoded_query}&num={limit}"
            logger.info(f"访问 Google 搜索页面: {search_url}")
            ...
            # 提取搜索结果（这里仍然使用 limit 作为 maxResults）
            ...
            if not no_save_state:
                try:
                    ...
                    with open(fingerprint_file, "w", encoding="utf-8") as f:
                        json.dump(
                            fingerprint_data, f, ensure_ascii=False, indent=2
                        )
